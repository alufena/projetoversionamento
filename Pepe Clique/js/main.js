...
function Stats() {
  ...
  this.showTimeLeft = function() {
    document.querySelector("#game-timeLeft h4").textContent = "Tempo restante: " + timeLeft;
  }
  this.setScore = function(scoreNumber) {
    score = scoreNumber;
    docScore.textContent = "Pontos: " + scoreNumber;
  }
  this.addScore = function(scoreToAdd) {
    score += scoreToAdd;
    docScore.textContent = "Pontos: " + score;
  }
  ...
  this.setHighscore = function(highscoreNumber) {
    if (highscoreNumber > highscore) {
      document.querySelector("#gameover-highscore").textContent = 'Novo recorde - ' + highscoreNumber + '! (Instead ' + highscore + ')'
  ...
  this.showHighscore = function() {
    document.querySelector("#game-highscore h4").textContent = "Pontuação máxima: " + highscore;
  }
...
(function() {
  ...
  const startRageMode = () => {
    ...
    sounds.rage.play();
    document.body.classList.add('jogar-modo-raiva');
  };

  const stopRageMode = () => {
    clearInterval(timerId);
    inProgress = false;
    playRageBtn.innerHTML = 'jogar modo raiva';
    sounds.rage.pause();
    document.body.classList.remove('jogar-modo-raiva');
  }
